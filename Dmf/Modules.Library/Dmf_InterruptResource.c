...
NTSTATUS 
InterruptResource_InterruptEnable(
...
Routine Description:
    ...
    Enable interrupt callback. Chains to Client.

Arguments:

    Interrupt - Handle to the associated WDF interrupt.
    AssociatedDevice - Handle to the associated WDF device.
...
--*/
{
    ...
    //
    DmfAssert(AssociatedDevice == DMF_ParentDeviceGet(*dmfModuleAddress));

    moduleConfig = DMF_CONFIG_GET(*dmfModuleAddress);

    // Call the optional PASSIVE_LEVEL Client Driver callback.
    //
    DmfAssert(moduleConfig->EvtInterruptResourceInterruptEnable != NULL);

    ntStatus = moduleConfig->EvtInterruptResourceInterruptEnable(*dmfModuleAddress);
...
EVT_WDF_INTERRUPT_ENABLE EvtWdfInterruptDisable;

NTSTATUS 
InterruptResource_InterruptDisable(
  _In_ WDFINTERRUPT Interrupt,
  _In_ WDFDEVICE AssociatedDevice
    )
/*++
...
Routine Description:
    ...
    Disable interrupt callback. Chains to Client.
...
--*/
{
    ...
    // Call the optional PASSIVE_LEVEL Client Driver callback.
    //
    DmfAssert(moduleConfig->EvtInterruptResourceInterruptDisable != NULL);

    ntStatus = moduleConfig->EvtInterruptResourceInterruptDisable(*dmfModuleAddress);
...
Arguments:
    ...
    Interrupt - Handle to the associated WDF interrupt.
    WdfDevice - Handle to the associated WDF device..
...
Routine Description:
    ...
    DPC callback for the associated interrupt.
...
/*++
  ...
  Arguments:
    ...
    Interrupt - Handle to the associated WDF interrupt.
    MessageId - Message number identifying the device's hardware interrupt message (if using MSI).
...
--*/
{
         ...
         (moduleConfig->EvtInterruptResourceInterruptDpc != NULL)))
    {
        ...
        if (moduleConfig->EvtInterruptResourceInterruptEnable != NULL)
        {
            interruptConfig.EvtInterruptEnable = InterruptResource_InterruptEnable;;
        }
        if (moduleConfig->EvtInterruptResourceInterruptDisable != NULL)
        {
            interruptConfig.EvtInterruptDisable = InterruptResource_InterruptDisable;
        }
...
--*/
{
    ...
    WdfInterruptDisable(moduleContext->Interrupt);
...
Routine Description:
    ...
    Enables interrupt. Interrupts are automatically enabled after D0Entry. But in some cases it is necessary
    to enable interrupts prior such as when downloading firmware during PrepareHardware.
...
--*/
{
    ...
    WdfInterruptEnable(moduleContext->Interrupt);
