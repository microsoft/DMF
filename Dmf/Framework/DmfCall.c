...
Routine Description:
    ...
    Invoke a given DMF Object's Client's Open Notification callback.
...
Return Value:
    ...
    NTSTATUS from the Client's callback.
...
--*/
{
    ...
    // NOTE: Generic handlers cannot validate because they do not receive a DMF Object.
    //
    ...
    // context means.
    //
    ntStatus = dmfObject->Callbacks.EvtModuleOnDeviceNotificationOpen(DmfModule);
...
Routine Description:
    ...
    Invoke a given DMF Object's Client's Close Notification callback.
...
--*/
{
    ...
    // NOTE: Generic handlers cannot validate because they do not receive a DMF Object.
    //
    DMF_HandleValidate_IsOpenedOrClosing(dmfObject);

    // Client notifications always get the Client Context. The Client decides what the
    // context means.
    //
    dmfObject->Callbacks.EvtModuleOnDeviceNotificationClose(DmfModule);
}
